...
# ================== Right Main Frame 1 =========================================
        ...
        #============================= Name Field =====================================
            ...
            self.name_lbl = Label(self.RightMainFrame,text="EMPNAME",anchor="w",font=("times new roman",12),bg="#ffe5b4")
        ...
        #============================ Buttons ===================================
            ...
            self.ButtonRF_3 = Button(self.RightMainFrame,bd=0, image = self.gmail_icon, activebackground="#ffe5b4",bg="#ffe5b4",cursor="hand2", command=self.link1)
            self.ButtonRF_3.place(x=130, y=530)

            self.ButtonRF_4 = Button(self.RightMainFrame,bd=0, image = self.insta_icon, activebackground="#ffe5b4",bg="#ffe5b4",cursor="hand2", command=self.link1)
            self.ButtonRF_4.place(x=230, y=530)

            self.ButtonRF_5 = Button(self.RightMainFrame,bd=0, image = self.twitter_icon, activebackground="#ffe5b4",bg="#ffe5b4",cursor="hand2", command=self.link1)
            ...
            self.ButtonRF_7 = Button(self.RightMainFrame,bd=5, relief=GROOVE, font=("iimes new roman",10,"bold"),text="User Sign In", activebackground="blue",activeforeground="yellow",fg="yellow",bg="blue",cursor="hand2", command=self.user)
...
# ================== Right Sub Frame 1 =========================================
            ...
            self.RightSubFrame_lbl1= Label(self.RightSubFrame, fg="#FFFF33",text="As Employee",font=("times new roman",18,"bold"),bg= "#B80F0A")
...
# ================== Left Main Frame 1 =========================================
    ...
    def verify(self):
        if self.otp == self.otp_entry.get():
            ...
            self.EmpID=self.name_txt.get()+str(random.randint(1000,40000))

            self.conn=sqlite3.connect("bakery.db")
            self.c=self.conn.cursor()
            self.c.execute("CREATE TABLE IF NOT EXISTS Emp(EmpID TEXT UNIQUE PRIMARY KEY NOT NULL ,EmpName TEXT NOT NULL, Email TEXT UNIQUE NOT NULL ,Country_Code TEXT NOT NULL, Country TEXT NOT NULL,Phone_No TEXT UNIQUE NOT NULL,Password TEXT NOT NULL)")
            self.find_user = ("SELECT * FROM Emp WHERE Email= ?  or Phone_No = ?")
            self.c.execute(str(self.find_user),(self.email_txt.get(),self.contact_txt.get()))
            results = (self.c).fetchall()
            if results:
                ...
                try:
                    self.c.execute("INSERT INTO Emp (EmpID ,EmpName, Email ,Country_Code, Country,Phone_No,Password ) VALUES (?,?,?,?,?,?,?)",
                    (self.EmpID,self.name_txt.get(),self.email_txt.get(),self.code.get(),self.count, self.contact_txt.get(),self.pass_txt.get()))
                    self.conn.commit()
                    self.c.close()
                    self.conn.close()
                    self.otp_entry.delete(0,END)
                    self.otp=None
                    return messagebox.showinfo("Successfull","Successfully Added Data, Now You Can Login"+" You ID :"+self.EmpID)
    ...
    def confirm(self):        
        ...
        self.conn = sqlite3.connect("bakery.db")
        self.c = self.conn.cursor()
        self.c.execute("SELECT Email from Emp WHERE Phone_No=" +
        ...
        if str(self.one_)==str(self.OTP_Forget):
            if len(str(self.passw))>=8:
                if self.passw == self.passconw:
                    y= f"UPDATE Emp SET Password = {str(self.passw)} WHERE Phone_No = {str(entry)}"
                    #print(str(y))
