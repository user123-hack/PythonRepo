...
#-----------------Main Screen Frames---------------------------------
        ...
        self.c.execute("SELECT * FROM Last_Login_User WHERE Email =\""+str(self.read1)+"\"")
    ...
    def logout(self):
        ...
        if a>0:
            ...
            self.conn=sqlite3.connect("bakery.db")
            self.c=self.conn.cursor()
            y="UPDATE Last_Login_User set last_login_time=\""+str(self.Time2)+"\", last_login_date=\""+str(self.today1)+"\" where Email =\""+str(self.read1)+"\""
            #print(y)
            self.c.execute(y)
            self.conn.commit()
            import loginu
